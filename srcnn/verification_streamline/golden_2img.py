#!/usr/bin/env python3
# Generate 2-image golden by running the SRCNN model twice (image 0 + image 1)
import numpy as np
import sys, os
sys.path.insert(0, os.path.dirname(__file__))

# Reuse logic from golden_srcnn.py by importing key bits
IMG = 150
NPIX = IMG * IMG  # 22500
...
# Load weights
l1_raw = read_hex_file("weight_l1_real.txt", 16)
l2_raw = read_hex_file("weight_l2_real.txt", 16)
l3_raw = read_hex_file("weight_l3_real.txt", 16)
...
# Load 2 images
all_in = read_hex_file("input_real.txt", 4)
print(f"Total input pixels: {len(all_in)}")

with open("golden_2img.txt", "w") as f:
    for img_idx in range(2):
        raw_in = all_in[img_idx*NPIX:(img_idx+1)*NPIX]
        assert len(raw_in) == NPIX, f"Image {img_idx}: expected {NPIX}, got {len(raw_in)}"
...
print("Wrote golden_2img.txt (45000 pixels)")
