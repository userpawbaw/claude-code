...
//  - L3_local_FSM + L3_PU
//  - intermid2_3 URAM R-port ?ìú?ùº?ù¥Î∏? (L2_top ?Ç¥ 4Í∞? URAM ?èô?ãú ?ùΩÍ∏?)
//  - URAM/output addr ?óÜ?ùå. final pixel 16bit + valid Í∑∏Î?Î°? ?Ö∏Ï∂?.
...
module L3_top #(
    ...
    input  wire                          i_rstn,
    input  wire                          i_start,
    input  wire                          i_image_bit,    // global FSM?óê?Ñú Ï£ºÏûÖ

    // Drives intermid2_3 URAM R-ports at L2_top (?èô?ùº addr/en?ùÑ 4 ch?óê broadcast)
    ...
    // Final pixel output (?ô∏Î∂? ?õÑÏ≤òÎ¶¨Î°? ÏßÅÏ†ë ?ùò?†§Î≥¥ÎÉÑ)
    ...
    // 4 ch Î™®Îëê ?èô?ùº addr / en ?úºÎ°? broadcast.
    // addr = {image_bit, FSM?ùò pixel_addr}
    ...
    // valid ?èô?ïÑ?ïà ?ïò?ÇòÎß? ?ì∞Î©? ?ê® (4Í∞? ?èô?ùº)
    wire [15:0] w_ch [0:3];
    ...
    wire [63:0] w_pu_input_data  = {w_ch[3], w_ch[2], w_ch[1], w_ch[0]}; // ?èô?ùº Ïª®Î≤§?Öò
    wire        w_pu_input_valid = i_l2_rd_valid[0]; // 4ch ?èôÍ∏∞Ïù¥ÎØ?Î°? [0]Îß? Ï∞∏Ï°∞
