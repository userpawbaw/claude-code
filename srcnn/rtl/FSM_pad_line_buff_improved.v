...
module FSM_pad #(
    ...
    parameter PAD             = 1,
    parameter I_NUM           = 152, // 150 + 2*PAD
    parameter O_NUM           = 150,
    parameter LB_LINE         = 3,
    parameter INIT_INPUT_SIZE = 456, // I_NUM * LB_LINE
    parameter SHIFT_ROW       = 150,
    parameter I_TOTAL         = 23104, // 152 * 152
...
always @(*) begin
    ...
    case(r_ps)
        S_IDLE :
            if(i_start || (layer_cnt != 2'd0) || (out_ch_cnt != 2'd0))
                r_ns = S_W_READ;
        S_W_READ :
            if(r_word_idx >= lut_w_words)
                r_ns = S_I_STREAM;
        S_I_STREAM :
            // 프레임 전체 크기(151, 151)까지 도달하면 DONE으로 천이
