...
//
// Design notes :
//   - Slice position: LSB-anchored, bits [WIN_BITS-1:0] = [95:0], 6 pixels.
...
//     Total = 1 + 37 = 38 emits, 2 + 37*4 = 150 cols per row. No boundary emit
//     across rows (row-end naturally lands on word_cnt=37).
//   - "col -1" slot inside slice (slice pixel 4, at K=0) holds the previous row's
//     col 151 — which is the right-pad zero from the L2 output. So lane 2's
//     left-neighbor input is naturally 0; no explicit mask required.
//
...
module line_buffer_wide_l3 #(
    ...
    // -----------------------------------------------------------------
    // 4. Window output : LSB-anchored slice (bits [WIN_BITS-1:0]) per row.
    //    Within each 96-bit row slice :
    //      slice[15:0]   = pixel 0 = lane 3 of newest word = col 4K+3.
    //      slice[63:48]  = pixel 3 = lane 0 of newest word = col 4K.
    //      slice[79:64]  = pixel 4 = lane 3 of prev word   = col 4K-1.
    //      slice[95:80]  = pixel 5 = lane 2 of prev word   = col 4K-2.
