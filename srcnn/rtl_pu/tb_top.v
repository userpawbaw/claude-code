`timescale 1ns / 1ps
// tb_top.v — rtl_pu 통합 TB (3 image 연속).
//   캡처: pack_L1_we (4 ch), pack_L2_we (out_ch_cnt 분기), o_pixel_valid (L3).
//   img_done 펄스마다 cap 인덱스 layer-wise 초기화.
module tb_top;
    reg  clk  = 0;
    reg  rstn = 0;
    reg  start = 0;
    ...
    // ── golden ──────────────────────────────────────────────────────
    reg [15:0] gold_L1  [0 : NIMG*4*NPIX - 1];
    reg [15:0] gold_L2  [0 : NIMG*2*NPIX - 1];
    reg [15:0] gold_out [0 : NIMG*1*NPIX - 1];

    // ── 캡처 ────────────────────────────────────────────────────────
    reg [15:0] cap_L1  [0 : NIMG*4*NPIX - 1];
    reg [15:0] cap_L2  [0 : NIMG*2*NPIX - 1];
    reg [15:0] cap_out [0 : NIMG*1*NPIX - 1];

    integer cap_L1_idx;     // pixel idx within current img (0..NPIX-1 step 4)
    integer cap_L2_oc0_idx, cap_L2_oc1_idx;
    integer cap_out_idx;
    integer cur_img;
    ...
    // img_done → reset per-img counters, advance cur_img
    always @(posedge clk) begin
        if (!rstn) cur_img <= 0;
    ...
    initial begin
        ...
        $display("");
        $display("=== L1  : %0d / %0d errors", errL1, NIMG*4*NPIX);
        $display("=== L2  : %0d / %0d errors", errL2, NIMG*2*NPIX);
    ...
    initial begin
        #5000000000;
        $display("TIMEOUT layer=%0d img=%0d capL1=%0d capL2 oc0=%0d oc1=%0d capOut=%0d all_done=%b",
                 dut.w_layer_cnt, dut.w_img_cnt, cap_L1_idx, cap_L2_oc0_idx, cap_L2_oc1_idx,
                 cap_out_idx, all_done);
        $finish;
    end
endmodule
