...
module top #(
    ...
    always @(posedge i_clk or negedge i_rstn) begin
        if (~i_rstn) begin
            for (hi = 0; hi < L3_IC; hi = hi + 1) r_l3_word[hi] <= 0;
            r_l3_half     <= 0;
            r_l3_in_valid <= 0;
    ...
    // L3 PU slot routing : MAX_CH=8 slots, only slot 0..L3_IC-1 driven.
    //   slot order : MSB (slot index 0 of PU) = ch 0, decreasing.
    //   PU.v expects: w_pu_data_l3[(MAX_CH-1-g)*64 +: 64] is ch g.
    ...
    // ------------------------------------------------------------------
    // L3 final output : oc 0 slot, 4 px × 16 bit + per-px valid.
    // ------------------------------------------------------------------
    assign o_pixel_valid = (w_layer_cnt == 2'd2) && w_pu_emit_valid && w_pu_oc_mask[0];
    assign o_pixel_data  = w_pu_pixel_data[0 + (4*16) +: 64];
